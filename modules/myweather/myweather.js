...
Module.register("myweather", {
	...
	defaults: {
		...
		iconTableColors: {
			"chanceflurries": "bbeadf",
			"chancerain": "d0dfff",
			"chancesleet": "cca3ff",
			"chancesnow": "e5d0ff",
			"chancetstorms": "0d6f88",
			"clear": "f2f1cd",
			"cloudy": "898f98",
			"flurries": "42b499",
			"fog": "4f5b66",
		...
		iconTableColorsNight: {
			"chanceflurries": "5c7d76",
			"chancerain": "667491",
			"chancesleet": "684c8b",
			"chancesnow": "796691",
			"chancetstorms": "052730",
			"clear": "858466",
			"cloudy": "3b3e42",
			"flurries": "19453b",
			"fog": "21262a",
		...
		if (this.config.forecasttable === 1) {
			...
			if (this.config.layout == "vertical") {
				...
				if (this.config.fctext == 1) {
					...
					forecastTextCell.setAttribute("colSpan", "20");
				...
				if (this.config.hourly == 1) {
					for (f in this.forecast) {
						...
						//hourCell.innerHTML = moment(forecast.hour, "HH:mm") + this.forecast.ampm;
				...
				if (this.config.hourly == 1) {
					...
					for (f in this.forecast) {
						...
						//hourCell.innerHTML = moment(forecast.hour, "HH:mm A") + forecast.ampm;
						//hourCell.innerHTML = moment(forecast.hour, "HH:mm a").format(this.config.timeFormat);
						hourCell.innerHTML = moment(forecast.hour, "HH:mm").format(this.config.timeFormat);
	...
	processWeather: function(data) {
		...
		if (data.response.hasOwnProperty("error")) {
			...
			if (this.config.enableColors === 1) {
				var currentColor = (sunrise < now && sunset > now) ? this.config.iconTableColors[data.current_observation.icon] : this.config.iconTableColorsNight[data.current_observation.icon];
				//if (sunriseSunsetDateObject == sunset) {
				//  var currentColor = this.config.iconTableColorsNight[data.current_observation.icon];
				//} else {
				//    var currentColor = this.config.iconTableColors[data.current_observation.icon];
				//}
				var url = "https://us.wio.seeed.io/v1/node/GroveLedWs2812D2/clear/60/" + currentColor + "?access_token=cd401a2aa8dd9d9629c666df86f2a46b";
				var sendCurrentColor = new XMLHttpRequest();
				sendCurrentColor.open("POST", url, true);
				sendCurrentColor.send();
			}
			var timeString;
			...
			this.weatherTypeTxt = "<img src='./modules/myweather/img/" + this.config.iconset + "/" +
			...
			for (i = this.config.fcdaystart, count = data.forecast.simpleforecast.forecastday.length; i < this.config.fcdaycount; i++) {
				...
				this.icon_url = "<img style='max-height:100%; max-width:100%; vertical-align:middle' src='./modules/myweather/img/" + this.config.iconset + "/" +
			...
			if (this.config.hourly == 1) {
				...
				for (i = 0, count = data.hourly_forecast.length; i < count; i++) {
					...
					var ttofd = hourlyforecast.FCTTIME.ampm;
					...
					this.ForecastIconUrl = "<img style='max-height:100%; max-width:100%; vertical-align:middle' src='./modules/myweather/img/" + this.config.iconset + "/" +
