#!/usr/bin/env bash
# This is an installer script for MagicMirror2 arm64 Version. It works well enough
...
cp config/custom.config.js config/config.js
...
if [[ $choice =~ ^[Yy]$ ]]; then
	...
	name=$(whoami)
	sudo su -c "env PATH=$PATH:/usr/bin pm2 startup linux -u "$name" --hp /home/"$name
	pm2 start ~/MagicMirror/installers/pm2_MagicMirror.json
	pm2 save
fi
echo " "
echo -e "\e[92mWe're ready! Run \e[1m\e[97mDISPLAY=:0 npm start\e[0m\e[92m from the ~/MagicMirror directory to start.\e[0m"
echo " "
echo " "
